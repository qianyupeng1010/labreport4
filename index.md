...
Type `vim vim ListExamples.java` and press `<enter>`, it allows me to see the text editor of ListExamples.java. Then type `41j`( do not need to press `<enter>`) , it moves down the cursor 41 lines down. Then type `11l` (do not need to press `<enter>`), it will move the cursor 11 characters to the right, and we reached the place where we need to edit. Then type `x`(do not need to `enter`), it will delete character `1` in the code. Then type `l`(do not need to press `<enter>`, this will 
